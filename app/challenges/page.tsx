...
export default function ChallengesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.challenger_id || !formData.challenged_id) {
      toast.error("Selecciona el retador y el retado");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({ challenger_id: "", challenged_id: "", message: "" });
        setShowForm(false);
        fetchChallenges();
      } else {
        const result = await response.json();
  ...
  return (
    <main className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      {showForm && (
        <Card className="p-6">
          <h2 className="text-xl font-bold mb-4">Crear Desafío</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1">Yo soy (Retador)</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Retaré a</label>
              <select
              ...
              <div className="flex justify-between items-start gap-4">
                <div className="flex-1">
                  <h3 className="font-bold text-lg">
                    {getPlayerName(challenge.challenger_id)} retó a{" "}
